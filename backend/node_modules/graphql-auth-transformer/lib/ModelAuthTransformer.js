      ...
      `);
        ...
        this.before = (ctx) => {
            ...
            if (!ctx.metadata.has(constants_1.AUTH_NON_MODEL_TYPES)) {
                ctx.metadata.set(constants_1.AUTH_NON_MODEL_TYPES, new Map());
            }
        };
        this.after = (ctx) => {
        ...
        this.object = (def, directive, ctx) => {
            ...
            this.protectGetQuery(ctx, graphql_transformer_common_1.ResolverResourceIDs.DynamoDBGetResolverResourceID(def.name.value), queryRules.get, def, modelConfiguration, true);
            this.protectListQuery(ctx, graphql_transformer_common_1.ResolverResourceIDs.DynamoDBListResolverResourceID(def.name.value), queryRules.list, def, modelConfiguration, undefined, true);
    ...
    propagateAuthDirectivesToNestedTypes(type, rules, ctx) {
        ...
        const getDirectivesToAdd = (nonModelName) => {
            ...
            if (seenNonModelTypes.has(nonModelName)) {
                const nonModelDirectives = seenNonModelTypes.get(nonModelName);
        ...
        for (const nonModelFieldType of nonModelFieldTypes) {
            ...
            if (directives.current.length > 0) {
                ...
                const hasIAM = directives.current.filter(directive => directive.name.value === 'aws_iam') || this.configuredAuthProviders.default === 'iam';
                if (hasIAM) {
...
Either make the field optional, set auth on the object and not the field, or disable subscriptions for the object (setting level to off or public)\n`);
                ...
                resolver.Properties.ResponseMappingTemplate = graphql_mapping_template_1.print(this.resources.operationCheckExpression(ctx.getMutationTypeName(), field));
            }
...
operations will be generated by the CLI.`);
    ...
    protectGetQuery(ctx, resolverResourceId, rules, parent, modelConfiguration, updateSchema) {
    ...
    protectListQuery(ctx, resolverResourceId, rules, parent, modelConfiguration, explicitOperationName = undefined, updateSchema = true) {
        const resolver = ctx.getResource(resolverResourceId);
        if (!rules || rules.length === 0 || !resolver) {
            return;
        }
    ...
    extendAuthRulesForAdminUI(rules) {
        if (this.isAdminUIEnabled()) {
...
found '${rule.provider}' assigned.`);
    ...
    doesTypeHaveRulesForOperation(def, operation) {
        const authDirective = def.directives.find(dir => dir.name.value === 'auth');
        if (!authDirective) {
        ...
        switch (operation) {
            case 'create':
                return (hasRulesForDefaultProvider(operationRules.create) || (operationRules.create.length === 0 && this.shouldAddDefaultAuthDirective()));
            case 'update':
                return (hasRulesForDefaultProvider(operationRules.update) || (operationRules.update.length === 0 && this.shouldAddDefaultAuthDirective()));
            case 'delete':
                return (hasRulesForDefaultProvider(operationRules.delete) || (operationRules.delete.length === 0 && this.shouldAddDefaultAuthDirective()));
            case 'get':
