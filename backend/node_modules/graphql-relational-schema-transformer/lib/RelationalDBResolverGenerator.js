...
class RelationalDBResolverGenerator {
    ...
    makeCreateRelationalResolver(type, mutationTypeName = 'Mutation') {
        ...
        const reqTemplate = graphql_mapping_template_1.print(graphql_mapping_template_1.compoundExpression([
            ...
            graphql_mapping_template_1.forEach(graphql_mapping_template_1.ref('entry'), graphql_mapping_template_1.ref(`ctx.args.create${tableName}Input.keySet()`), [
                graphql_mapping_template_1.set(graphql_mapping_template_1.ref('discard'), graphql_mapping_template_1.ref(`cols.add($entry)`)),
                graphql_mapping_template_1.set(graphql_mapping_template_1.ref('discard'), graphql_mapping_template_1.ref(`vals.add("'$ctx.args.create${tableName}Input[$entry]'")`)),
            ]),
    ...
    makeGetRelationalResolver(type, queryTypeName = 'Query') {
        ...
        const reqTemplate = graphql_mapping_template_1.print(graphql_mapping_template_1.compoundExpression([
            RelationalDBMappingTemplate_1.RelationalDBMappingTemplate.rdsQuery({
                statements: graphql_mapping_template_1.list([graphql_mapping_template_1.str(selectSql)]),
            }),
        ]));
    ...
    makeUpdateRelationalResolver(type, mutationTypeName = 'Mutation') {
        ...
        const reqTemplate = graphql_mapping_template_1.print(graphql_mapping_template_1.compoundExpression([
            ...
            graphql_mapping_template_1.forEach(graphql_mapping_template_1.ref('entry'), graphql_mapping_template_1.ref(`ctx.args.update${tableName}Input.keySet()`), [
                graphql_mapping_template_1.set(graphql_mapping_template_1.ref('discard'), graphql_mapping_template_1.ref(`updateList.put($entry, "'$ctx.args.update${tableName}Input[$entry]'")`)),
            ]),
    ...
    makeListRelationalResolver(type, queryTypeName = 'Query') {
        const fieldName = graphql_transformer_common_1.graphqlName(GRAPHQL_RESOLVER_OPERATION.List + graphql_transformer_common_1.plurality(graphql_transformer_common_1.toUpper(type)));
    ...
    getTableName(type) {
        return graphql_transformer_common_1.toUpper(type);
    }
    getFieldName(type, operationType) {
        const tableName = this.getTableName(type);
        return graphql_transformer_common_1.graphqlName(`${operationType}${tableName}`);
    }
    getTablePrimaryKey(type) {
        return this.typePrimaryKeyMap.get(type);
    }
    isPrimaryKeyAStringType(type) {
        return this.typePrimaryKeyTypeMap.get(type).includes('String');
    }
    generateSelectStatement(type) {
        const tableName = this.getTableName(type);
        return `SELECT * FROM ${tableName}`;
    }
    generateSelectByPrimaryKeyStatement(type, operationType) {
        const tableName = this.getTableName(type);
    ...
    generateInsertStatement(type) {
        const tableName = this.getTableName(type);
        return `INSERT INTO ${tableName} $colStr VALUES $valStr`;
    }
    generateUpdateStatement(type) {
        const tableName = this.getTableName(type);
        const primaryKey = this.getTablePrimaryKey(type);
        if (this.isPrimaryKeyAStringType(type)) {
...
var GRAPHQL_RESOLVER_OPERATION;
(function (GRAPHQL_RESOLVER_OPERATION) {
    GRAPHQL_RESOLVER_OPERATION["Create"] = "create";
    GRAPHQL_RESOLVER_OPERATION["Delete"] = "delete";
    GRAPHQL_RESOLVER_OPERATION["Get"] = "get";
    GRAPHQL_RESOLVER_OPERATION["List"] = "list";
    GRAPHQL_RESOLVER_OPERATION["Update"] = "update";
})(GRAPHQL_RESOLVER_OPERATION || (GRAPHQL_RESOLVER_OPERATION = {}));
