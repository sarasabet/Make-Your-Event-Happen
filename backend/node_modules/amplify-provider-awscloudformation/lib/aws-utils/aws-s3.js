...
class S3 {
    ...
    async getAllObjectVersions(bucketName, options = null) {
        const result = await paged_call_1.pagedAWSCall(async (param, nextToken) => {
            ...
            Bucket: bucketName,
            ...options,
        }, (response) => { var _a; return (_a = response.Versions) === null || _a === void 0 ? void 0 : _a.map(({ Key, VersionId }) => ({ Key, VersionId })); }, async (response) => (response === null || response === void 0 ? void 0 : response.IsTruncated) ? { KeyMarker: response.NextKeyMarker, VersionIdMarker: response.NextVersionIdMarker, Prefix: response.Prefix }
            : undefined);
        return result;
    }
