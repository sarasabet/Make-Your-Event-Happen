...
const setup_new_user_1 = __importDefault(require("./setup-new-user"));
...
async function enableServerlessContainers(context) {
    ...
    const { ServerlessContainers } = await inquirer_1.prompt({
        type: 'confirm',
...
function normalizeInputParams(context) {
    ...
    if (context.exeInfo.inputParams) {
        if (context.exeInfo.inputParams[constants_1.default.ProviderName]) {
            ...
            for (let alias of constants_1.default.Aliases) {
...
async function initialize(context, authConfig) {
    ...
    if (!awsConfigInfo.configValidated) {
        ...
        const { retryConfirmation } = await inquirer_1.prompt(configuration_questions_1.retryAuthConfig);
...
async function setProjectConfigAction(context) {
    if (context.exeInfo.inputParams[constants_1.default.ProviderName]) {
        ...
        if (context.exeInfo.awsConfigInfo.configLevel === 'project') {
            const answer = await inquirer_1.prompt(configuration_questions_1.updateOrRemoveQuestion);
            context.exeInfo.awsConfigInfo.action = answer.action;
        }
        else {
            const answer = await inquirer_1.prompt(configuration_questions_1.createConfirmQuestion);
...
async function confirmProjectConfigRemoval(context) {
    if (!context.exeInfo.inputParams.yes) {
        const answer = await inquirer_1.prompt(configuration_questions_1.removeProjectComfirmQuestion);
...
async function promptForAuthConfig(context, authConfig) {
    ...
    if (availableProfiles && availableProfiles.length > 0) {
        ...
        if (authConfig === null || authConfig === void 0 ? void 0 : authConfig.type) {
            ...
            try {
                const appId = resolve_appId_1.resolveAppId(context);
                isAdminApp = ((_a = (await admin_helpers_1.isAmplifyAdminApp(appId))) === null || _a === void 0 ? void 0 : _a.isAdminApp) || false;
            }
            catch (_b) {
                isAdminApp = false;
            }
        ...
        if (authType === 'profile') {
            ...
            answers = await inquirer_1.prompt(configuration_questions_1.profileNameQuestion(availableProfiles, awsConfigInfo.config.profileName));
    ...
    answers = await inquirer_1.prompt(configuration_questions_1.accessKeysQuestion(awsConfigInfo.config.accessKeyId ? utility_obfuscate_1.default.obfuscate(awsConfigInfo.config.accessKeyId) : constants_1.default.DefaultAWSAccessKeyId, awsConfigInfo.config.secretAccessKey
...
function persistLocalEnvConfig(context) {
    ...
    if (appId && admin_helpers_1.doAdminTokensExist(appId)) {
        awsConfigInfo = {
            configLevel: 'amplifyAdmin',
            config: {},
        };
...
function getConfigForEnv(context, envName) {
    var _a;
    const projectConfigInfo = ((_a = context === null || context === void 0 ? void 0 : context.exeInfo) === null || _a === void 0 ? void 0 : _a.awsConfig) || {
        configLevel: 'general',
        config: {},
    };
    const dotConfigDirPath = amplify_cli_core_1.pathManager.getDotConfigDirPath();
...
async function loadConfigurationForEnv(context, env, appId) {
    ...
    if (authType.type === 'admin') {
        ...
        try {
            awsConfig = await admin_helpers_1.getTempCredsWithAdminTokens(context, appId);
        }
        catch (e) {
    ...
    if (!configSource) {
        ...
        if (context.exeInfo.inputParams.yes) {
            ...
            if (answer.setupNewUser) {
                context.newUserInfo = await setup_new_user_1.default.run(context);
            }
        }
    }
}
...
async function getAwsConfig(context) {
    ...
    if (httpProxy) {
        awsConfig = {
            ...awsConfig,
            httpOptions: { agent: proxy_agent_1.default(httpProxy) },
        };
    }
    return awsConfig;
}
...
async function determineAuthFlow(context, projectConfig) {
    ...
    let adminAppConfig;
    try {
        appId = resolve_appId_1.resolveAppId(context);
        if (appId) {
            adminAppConfig = await admin_helpers_1.isAmplifyAdminApp(appId);
            if (adminAppConfig.isAdminApp && adminAppConfig.region) {
                region = adminAppConfig.region;
                if (admin_helpers_1.doAdminTokensExist(appId) && (projectConfig === null || projectConfig === void 0 ? void 0 : projectConfig.configLevel) === 'amplifyAdmin') {
    ...
    if ((_g = (_f = context === null || context === void 0 ? void 0 : context.exeInfo) === null || _f === void 0 ? void 0 : _f.inputParams) === null || _g === void 0 ? void 0 : _g.yes) {
        ...
        context.print.info(`Access keys for continuous integration can be configured with headless paramaters: ${chalk_1.default.green(docsUrl)}`);
...
async function askAuthType(isAdminAvailable = false) {
    ...
    const { authChoice } = await inquirer_1.prompt(configuration_questions_1.authTypeQuestion(choices));
    return authChoice;
}
