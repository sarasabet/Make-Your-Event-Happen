...
exports.importedAuthEnvInit = exports.importResource = void 0;
...
const appClientsOAuthPropertiesMatching = async (context, appClientWeb, appClientNative, printErrors = true) => {
    ...
    let propertiesMatching = supportedIdentityProvidersMatching &&
        callbackUrlMatching &&
        logoutUrlsMatching &&
...
const importedAuthEnvInit = async (context, resourceName, resource, resourceParameters, providerName, providerUtils, currentEnvSpecificParameters, isInHeadlessMode, headlessParams) => {
    ...
    if (isPulling) {
        ...
        if (sourceEnvParams) {
            const { importExisting } = await enquirer_1.default.prompt({
                name: 'importExisting',
                type: 'confirm',
                message: messages_1.importMessages.Questions.ImportPreviousResource(resourceName, sourceEnvParams.userPoolId, context.exeInfo.sourceEnvName),
    ...
    try {
        ...
        if (error.name === 'ResourceNotFoundException') {
            context.print.error(messages_1.importMessages.UserPoolNotFound(currentEnvSpecificParameters.userPoolName, currentEnvSpecificParameters.userPoolId));
...
const headlessImport = async (context, cognito, identity, providerName, resourceName, resource, resourceParameters, headlessParams) => {
    ...
    try {
        ...
        if (error.name === 'ResourceNotFoundException') {
            throw new Error(messages_1.importMessages.UserPoolNotFound(currentEnvSpecificParameters.userPoolName, currentEnvSpecificParameters.userPoolId));
        }
        throw error;
    }
    const validationResult = await validateUserPool(cognito, identity, questionParameters, answers, currentEnvSpecificParameters.userPoolId);
    ...
    if (resourceParameters.authSelections === 'identityPoolAndUserPool') {
        const identityPools = questionParameters.validatedIdentityPools.filter(idp => idp.identityPool.IdentityPoolId === currentEnvSpecificParameters.identityPoolId);
        if (identityPools.length !== 1) {
            throw new Error(messages_1.importMessages.IdentityPoolNotFound(currentEnvSpecificParameters.identityPoolName, currentEnvSpecificParameters.identityPoolId));
        }
        answers.identityPoolId = identityPools[0].identityPool.IdentityPoolId;
        answers.identityPool = identityPools[0].identityPool;
        answers.identityProviders = identityPools[0].providers;
