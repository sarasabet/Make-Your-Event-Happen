...
exports.updateCFNFileForResourcePermissions = exports.migrate = exports.updateWalkthrough = exports.createWalkthrough = void 0;
...
async function createWalkthrough(context, templateParameters) {
    ...
    constants_1.advancedSettingsList.forEach(setting => context.print.info('- '.concat(setting)));
    context.print.info('');
...
async function updateWalkthrough(context, lambdaToUpdate) {
    ...
    const functionRuntime = context.amplify.readBreadcrumbs(constants_2.category, functionParameters.resourceName).functionRuntime;
    const cfnParameters = amplify_cli_core_1.JSONUtilities.readJson(path_1.default.join(resourceDirPath, constants_1.parametersFileName), { throwIfNotExist: false }) || {};
    ...
    if (selectedSettings.includes(constants_1.cronJobSetting)) {
    ...
    if (selectedSettings.includes(constants_1.lambdaLayerSetting)) {
        const currentFunctionParameters = amplify_cli_core_1.JSONUtilities.readJson(path_1.default.join(resourceDirPath, constants_1.functionParametersFileName), { throwIfNotExist: false }) || {};
