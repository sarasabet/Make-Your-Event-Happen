    ...
    for (const selectedCategory of selectedCategories) {
        ...
        if (selectedCategory === 'storage' && 'api' in amplifyMeta) {
            ...
            if (serviceName === "Lambda") {
                resourcesList = resourcesList.filter(resourceName => resourceName !== resourceNameToUpdate && amplifyMeta[selectedCategory][resourceName].service === serviceName);
            }
            else {
        ...
        try {
            ...
            for (let resourceName of selectedResources) {
                ...
                if (isMobileHubImportedResource) {
                    ...
                    const currentPermissions = permissionMapUtils_1.fetchPermissionsForResourceInCategory(currentPermissionMap, selectedCategory, resourceName);
...
const selectResourcesInCategory = (choices, currentPermissionMap, category) => ({
    ...
    choices,
    validate: answers => (lodash_1.default.isEmpty(answers) ? 'You must select at least one resource' : true),
...
async function getResourcesForCfn(context, resourceName, resourcePolicy, appsyncResourceName, selectedCategory) {
    ...
    const cfnResources = resourceAttributes.map(attributes => attributes.resourceName && attributes.resourceName.endsWith(constants_2.appsyncTableSuffix)
        ? {
