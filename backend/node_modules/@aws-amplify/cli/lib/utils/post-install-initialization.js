...
const postInstallInitialization = async () => {
    ...
    await Promise.all(copyPkgAssetRegistry.map(async (packageName) => {
        ...
        await Promise.all(pluginArtifactPaths.map(assetPath => fs.copy(path.join(resolvePackageRoot(packageName), assetPath), path.join(amplify_cli_core_1.pathManager.getAmplifyPackageLibDirPath(packageName), assetPath))));
    }));
