...
function getConnectedFieldV2(field, model, connectedModel, directiveName) {
    const connectionInfo = fieldUtils_1.getDirective(field)(directiveName);
    if (!connectionInfo) {
        throw new Error(`The ${field.name} on model ${model.name} is not connected`);
    }
    if (connectionInfo.name === 'belongsTo') {
        let connectedFieldBelongsTo = process_belongs_to_1.getBelongsToConnectedField(field, model, connectedModel);
    ...
    if (connectionFields || directiveName === 'hasOne') {
        ...
        const connectedFieldName = indexDirective
            ...
            : process_connections_1.DEFAULT_HASH_KEY_FIELD;
