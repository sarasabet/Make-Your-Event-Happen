...
class AppSyncModelVisitor extends visitor_plugin_common_1.BaseVisitor {
    ...
    generateIntermediateModel(firstModel, secondModel, firstField, secondField, relationName) {
    ...
    convertManyToManyDirectives(contexts) {
        contexts.forEach(context => {
            ...
            if (directiveIndex > -1) {
                ...
                context.field.type = graphql_transformer_common_1.graphqlName(graphql_transformer_common_1.toUpper(context.directive.arguments.relationName));
    ...
    processManyToManyDirectives() {
            ...
            model.fields.forEach(field => {
                field.directives.forEach(dir => {
                    if (dir.name === 'manyToMany') {
                        let relationName = graphql_transformer_common_1.graphqlName(graphql_transformer_common_1.toUpper(dir.arguments.relationName));
        ...
        manyDirectiveMap.forEach((value, key) => {
            ...
            let intermediateModel = this.generateIntermediateModel(value[0].model, value[1].model, value[0].field, value[1].field, graphql_transformer_common_1.graphqlName(graphql_transformer_common_1.toUpper(value[0].directive.arguments.relationName)));
    ...
    processConnectionDirectivesV2() {
        this.processManyToManyDirectives();
        Object.values(this.modelMap).forEach(model => {
            model.fields.forEach(field => {
                const connectionInfo = process_connections_v2_1.processConnectionsV2(field, model, this.modelMap);
                if (connectionInfo) {
                    if (connectionInfo.kind === process_connections_1.CodeGenConnectionType.HAS_MANY) {
                        ...
                        fieldUtils_1.addFieldToModel(model, {
            ...
            model.fields = model.fields.filter(field => {
                ...
                const connectionInfo = field.connectionInfo;
                if (modelTypes.includes(fieldType) && connectionInfo === undefined) {
                    warn_1.printWarning(`Model ${model.name} has field ${field.name} of type ${field.type} but its not connected. Add the appropriate ${field.isList ? '@hasMany' : '@hasOne'}/@belongsTo directive if you want to connect them.`);
                    return false;
                }
                return true;
            });
    ...
    processV2KeyDirectives() {
        Object.values(this.modelMap).forEach(model => {
            process_primary_key_1.processPrimaryKey(model);
            process_index_1.processIndex(model);
        });
    }
