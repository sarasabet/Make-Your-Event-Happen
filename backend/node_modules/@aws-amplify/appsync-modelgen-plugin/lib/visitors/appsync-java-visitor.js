...
class AppSyncModelJavaVisitor extends appsync_visitor_1.AppSyncModelVisitor {
    ...
    generateStepBuilderInterfaces(model, isModel = true) {
        ...
        const requiredInterfaces = nonNullableFields.filter((field) => !(isModel && this.READ_ONLY_FIELDS.includes(field.name)));
    ...
    generateBuilderClass(model, classDeclaration, isModel = true) {
        const nonNullableFields = this.getWritableFields(model).filter(field => this.isRequiredField(field));
        const nullableFields = this.getWritableFields(model).filter(field => !this.isRequiredField(field));
        const stepFields = nonNullableFields.filter((field) => !(isModel && this.READ_ONLY_FIELDS.includes(field.name)));
    ...
    return this;`;
            const idComment = ts_dedent_1.default `
    @param id id
    @return Current Builder instance, for fluent method chaining`;
            builderClassDeclaration.addClassMethod('id', this.getStepInterfaceName('Build'), visitor_plugin_common_1.indentMultiline(idBuildStepBody), [{ name: 'id', type: 'String' }], [], 'public', {}, [], [], idComment);
        }
    ...
    generateBuilderMethod(model, classDeclaration, isModel = true) {
        const requiredFields = this.getWritableFields(model).filter(field => !field.isNullable && !(isModel && this.READ_ONLY_FIELDS.includes(field.name)));
