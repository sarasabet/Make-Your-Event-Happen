...
class SearchableModelTransformer extends graphql_transformer_core_1.Transformer {
    ...
    generateSearchableInputs(ctx, def) {
        ...
        inputs
            .filter((input) => !this.typeExist(`Searchable${input}FilterInput`, ctx))
            .map((input) => definitions_1.makeSearchableScalarInputObject(input))
            .forEach((node) => ctx.addInput(node));
